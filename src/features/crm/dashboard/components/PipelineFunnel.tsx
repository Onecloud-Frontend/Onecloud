...
export const PipelineFunnel: React.FC<PipelineFunnelProps> = ({ data }) => {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200/60 p-6 shadow-[0_1px_2px_rgba(0,0,0,0.02)] flex flex-col h-full">
      <div className="mb-5">
        <h2 className="text-base font-bold text-[#0b1f4d]">Sales Pipeline</h2>
        <p className="text-xs text-slate-500 font-medium mt-1">Current opportunities by stage</p>
      </div>
      
      <div className="flex-1 flex flex-col justify-center gap-3">
        {data.map((stage, index) => {
          const widthPercentage = Math.max(20, 100 - (index * 15)); // Funnel shape
          ...
          return (
            <div key={stage.id} className="relative w-full flex justify-center">
              ...
              >
                <div className="absolute inset-0 bg-white opacity-40 mix-blend-overlay rounded-lg pointer-events-none"></div>
                <span className="relative text-[13px] font-bold text-slate-700 z-10">{stage.name}</span>
